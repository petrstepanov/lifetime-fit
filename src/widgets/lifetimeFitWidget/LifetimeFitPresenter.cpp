...
LifetimeFitPresenter::LifetimeFitPresenter(LifetimeFitView* view) : AbstractPresenter<Model, LifetimeFitView>(view) {
    // Need to instantinate RooRealVarView model for source Contribution
//    Model* model = getModel();
}

Model* LifetimeFitPresenter::instantinateModel(){
    return Model::getInstance();
}

void LifetimeFitPresenter::onInitModel() {
    std::cout << "LifetimeFitPresenter::onInitModel" << std::endl;
}

void LifetimeFitPresenter::addEventListeners(){
    LifetimeFitView* view = getView();
    Model* model = getModel();

    class HistogramImportedEventListener : public EventHandler<HistogramImportedEvent> {
      public:
        HistogramImportedEventListener(LifetimeFitView* v) : view(v) {}
    ...
    class SourceHistogramImportedEventListener : public EventHandler<SourceHistogramImportedEvent> {
      public:
        SourceHistogramImportedEventListener(LifetimeFitView* v, Model* m) : view(v), model(m) {}
    ...
    class IsTwoDetectorEventListener : public EventHandler<IsTwoDetectorEvent> {
      public:
        IsTwoDetectorEventListener(LifetimeFitView* v) : view(v) {}
...
void LifetimeFitPresenter::onFitSpectrumClicked(){
    // Start Tracking Time
...
void LifetimeFitPresenter::onSaveImageClicked() {
    Model* model = getModel();
    LifetimeFitView* view = getView();
...
void LifetimeFitPresenter::onSaveResultsClicked() {
    Model* model = getModel();
    LifetimeFitView* view = getView();
...
void LifetimeFitPresenter::onClearResultsClicked() {
    LifetimeFitView* view = getView();
